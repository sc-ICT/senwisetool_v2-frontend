...
import type { FormGlobalConfig } from "@/types/form";
import { AttachmentsSection } from "./form-attachments-section";

interface FormGlobalConfigPanelProps {
  formId: number;
  config: FormGlobalConfig;
  isPending: boolean;
  onClose: () => void;
  onSubmit: (config: FormGlobalConfig) => void;
  onSaveConfig?: (config: FormGlobalConfig) => Promise<boolean>;
}
...
export function FormGlobalConfigPanel({
  formId,
  config,
  isPending,
  onClose,
  onSubmit,
  onSaveConfig,
}: FormGlobalConfigPanelProps) {
  const [localConfig, setLocalConfig] = useState<FormGlobalConfig>(() =>
    normalizeFormGlobalConfig(config),
  );
  ...
  const updateGeolocation = (
    patch: Partial<FormGlobalConfig["geolocation"]>,
  ) => {
  ...
  const updateMapping = (patch: Partial<FormGlobalConfig["mapping"]>) => {
    setLocalConfig((current) => ({
      ...current,
      mapping: {
        ...current.mapping,
  ...
  const updateMonitoring = (
    patch: Partial<FormGlobalConfig["agent_monitoring"]>,
  ) => {
    setLocalConfig((current) => ({
      ...current,
      agent_monitoring: {
  ...
  const updateAntiFraud = (patch: Partial<FormGlobalConfig["anti_fraud"]>) => {
    setLocalConfig((current) => ({
      ...current,
      anti_fraud: {
        ...current.anti_fraud,
  ...
  const updateOffline = (patch: Partial<FormGlobalConfig["offline"]>) => {
    setLocalConfig((current) => ({
      ...current,
      offline: {
        ...current.offline,
  ...
  const updateMedia = (patch: Partial<FormGlobalConfig["media"]>) => {
    setLocalConfig((current) => ({
      ...current,
      media: {
        ...current.media,
  ...
  const updateAttachments = (
    patch: Partial<FormGlobalConfig["attachments"]>,
  ) => {
    setLocalConfig((current) => ({
      ...current,
      attachments: {
      ...
      >
          ...
          <div>
            ...
            >
              ...
              <div>
                ...
                >
                  Configurez le comportement général du formulaire.
                </div>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <ConfigSectionHeader
            ...
            description="Autorisez les fichiers qui seront associés au formulaire."
          ...
          {openSections.attachments && (
            <div style={sectionContentStyle}>
              <ToggleRow
                ...
                description="Permet d'associer des fichiers au formulaire."
              ...
              {localConfig.attachments.enabled && (
                <AttachmentsSection
                  ...
                  disabled={isPending}
                  listFiles={async () => {
                    const response = await formService.listFiles(formId);

                    return response.data?.items ?? [];
                  }}
                  uploadFile={async (file) => {
                    const response = await formService.uploadFile(formId, file);

                    if (!response.data) {
                      throw new Error("Le fichier uploadé est introuvable.");
                    }

                    return response.data;
                  }}
                  deleteFile={async (fileId) => {
                    await formService.deleteFile(formId, fileId);
                  }}
                  ...
                  emptyMessage="Aucun fichier attaché à ce formulaire."
                />
              )}
            </div>
          )}
...
function normalizeFormGlobalConfig(
  config: Partial<FormGlobalConfig> | undefined,
): FormGlobalConfig {
  return {
    collection: {
